<div class="px-2">
    ...
    <div class="flex justify-between mb-6">
        <h1 class="sm:text-3xl text-xl text-newgray-900 dark:text-newgray-300 font-semibold ">In the news</h1>

        <a href="{{ url('/cms/addnews') }}" class="inline-flex sm:px-12 px-8 sm:py-2 py-1 rounded dark:hover:bg-newgray-900 dark:hover:border-gray-200 dark:hover:text-gray-200 hover:bg-white hover:text-newgray-900 border hover:border-newgray-900 bg-newgray-900 dark:bg-gray-100 text-newgray-100 dark:text-newgray-900">
            Add
        </a>
    </div>
                    ...
                    <thead >
                        <tr >
                            ...
                            <th class="px-4 py-3 bg-gray-50 dark:bg-opacity-10  dark:text-white text-left text-xs font-medium text-gray-500 uppercase tracking-wider  sm:w-2/12 w-0">
                                <a class="hidden sm:block">Source</a>
                            </th>
                            <th class="px-4 py-3 bg-gray-50 dark:bg-opacity-10  dark:text-white text-left text-xs font-medium text-gray-500 uppercase tracking-wider  sm:w-2/12 w-0">
                                <a class="hidden sm:block">publish date</a>
                            </th>
                            <th wire:click='sortingField("is_active")' class=" cursor-pointer px-4 py-3 bg-gray-50 dark:bg-opacity-10  dark:text-white text-center font-medium text-gray-500 uppercase tracking-wider  sm:w-2/12 w-0">
                    ...
                    <tbody class="bg-white dark:bg-opacity-20 dark:text-white divide-y divide-gray-200 dark:divide-gray-900">
                        ...
                        <tr>
                            <td class="px-6 py-4 break-words text-sm font-bold text-newgray-700 dark:text-gray-300">
                                <a href="{{ url('/cms/news/'.$item->id) }}">{{ $item->titleID }}</a>
                            </td>



                            <td class="px-4 py-4 break-words text-sm font-bold text-newgray-700 dark:text-gray-300">
                                <a>{{ $item->sourcename }}</a>
                            </td>
                            <td class="px-4 py-4 break-words text-sm font-bold text-newgray-700 dark:text-gray-300">
                                <a>{{ date('d-M-Y', strtotime($item->publishdate)); }}</a>
                            </td>
                            ...
                            <td colspan="2" class=" break-words text-sm text-gray-500 dark:text-gray-300 px-6">
                                <div class="relative flex justify-end" x-data="{ open: false }">
                                    ...
                                    <ul
                                        ...
                                        <a data-turbolinks="false" href="{{ url('/cms/news/'.$item->id) }}"><li class="block hover:bg-gray-200 cursor-pointer py-1 mt-2 px-4 dark:text-gray-500" @click.away="open = false">Edit</li></a>
